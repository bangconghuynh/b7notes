...
\section{Preliminaries}
	...
	\subsection{Some Important Abstract Groups}
		...
		\subsubsection{Linear Groups}
			...
			It is indeed a fairly abstract group, and it might therefore be easier to think about $\symup{GL}(V)$ as the group of all invertible linear transformations acting on a particular vector space and mapping each vector to another vector.
			...
			\begin{proposition}
				The groups $\symup{GL}(V)$ and $\symup{GL}_n(\symbb{F})$ are isomorphic with the isomorphism given by $\theta \mapsto \symbfit{M}_{\theta}$ where $\symbfit{M}_{\theta}$ are $n \times n$ invertible matrices.
			\end{proposition}