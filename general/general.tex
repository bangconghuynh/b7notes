...
\section{Preliminaries}
	...
	\subsection{Some Important Abstract Groups}
		...
		We provide a quick survey of some important abstract groups to which many of the commonly encountered concrete groups are isomorphic, and quite a few of which have far-reaching implications in quantum mechanics.
		...
		\subsubsection{Linear Groups}
			...
			In this \namecref{sec:lineargroups}, we write $\symbb{F}$ to denote a field that can be either $\symbb{C}$ or $\symbb{R}$.
			...
			We often think about $V$ as the space of geometrical vectors in some dimensions, due mainly to its name, but $V$ can be a much more general space than that.
			The following definitions of some concrete vector spaces should make this clear.
			
			\begin{definition}[Coordinate space]
				For a positive integer $n$, the set of all $n$-tuples of elements of $\symbb{F}$, $\symbfit{v} = (x_1, x_2, \ldots, x_n)$,
				forms an $n$-dimensional vector space over $\symbb{F}$ called a \emph{coordinate space} and denoted $\symbb{F}^n$.
			\end{definition}
			...
			\begin{example}\leavevmode
				\begin{enumerate}[label=(\alph*)]
					\item The set of all maps $f: \symbb{R} \rightarrow \symbb{R}$ given by $f: x \mapsto \sum_{i=0}^{n}a_i x^i$ for $a_i \in \symbb{R}$ is a function space of polynomials of degrees no more than $n$.
					\item The set of all maps $f: \symbb{R} \times \symbb{R} \rightarrow \symbb{R}$ given by $f: (\theta, \phi) \mapsto \sum_{l=0}^{n} \sum_{m=-l}^{l} a_{l}^{m} Y_{l}^{m}(\theta, \phi)$ for $a_{l}^{m} \in \symbb{R}$ and $Y_{l}^{m}$ a spherical harmonic function of degree $l$ and order $m$ is a function space.
				\end{enumerate}
			\end{example}
		
			\begin{remark}
				Whenever we think about the representation of a certain symmetry operation as a matrix, this matrix always has to be written with respect to a certain basis set which spans a particular function space.
			\end{remark}
			...
			Now, if $V$ is a vector space of finite dimensions, say $n$, over $\symbb{F}$, then we can choose a basis $\left\lbrace \symbf{e}_1, \symbf{e}_2, \ldots, \symbf{e}_n \right\rbrace$ for $V$ over $\symbb{F}$ such that each vector $\symbfit{v} \in V$ can be written as $\symbfit{v} = \sum_{i=1}^{n}\symbf{e}_i v_i$.
