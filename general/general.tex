...
\section{Preliminaries}
	...
	\subsection{Group Homomorphisms}
		...
		\begin{example}\leavevmode
			...
			\begin{enumerate}[label=(\alph*)]
				\item \label{exp:homomorphism.trivial} The map $\varphi: G \rightarrow H$ defined by $\varphi: g \mapsto e_H$ is a group homomorphism.
		...
		\begin{definition}[Kernel]
			\label{def:imker}
			If $\varphi: G \rightarrow H$ is a group homomorphism, then the \emph{kernel} of $\varphi$, denoted by $\ker\varphi$, is the subset $\{g \in G : \varphi: g \mapsto e_H \}$.
		\end{definition}
	
		\begin{example}
			Consider the group homomorphisms defined in \cref{exp:homomorphism}.
			In \labelcref{exp:homomorphism.trivial}, the kernel of $\varphi$ is the whole group $G$.
			In \labelcref{exp:homomorphism.exp}, the kernel of $\varphi$ is the trivial set $\{0\}$.
		\end{example}
	
		\begin{proposition}
			If $\varphi: G \rightarrow H$ is a group homomorphism, then $\ker\varphi$ is a subgroup of $G$.
		\end{proposition}
	...
	\subsection{Vector Space}
		...
		A basis is therefore crucial in the specification of any representation and in determining if two representations are identical or only equivalent.


	\subsection{Some Important Abstract Groups}
	
		We can now provide a quick survey of some important abstract groups to which many of the commonly encountered concrete groups are isomorphic, and quite a few of which have far-reaching implications in quantum mechanics.
		The abstractness of these groups does make them quite hard to be understood at first, but having at least a notion of them helps us think about the zoo of finite and infinite groups more systematically.
		
		\subsubsection{The General and Special Linear Groups}
			\label{sec:generalspeciallineargroups}
			
			A lot of symmetry operations are actually linear maps acting on a certain vector space.
			The set of all such maps turns out to form an infinite group called the \emph{general linear group}.
			We first introduce this group in its most abstract sense and then relate it to more concrete groups of square matrices via group isomorphisms.
			This group plays a crucial role in the formalism of representation theory, and having at least a brief acknowledgement of its existence does help clarify quite a lot of ideas in the representations of groups which we will explore later on.
			...
			It is indeed a very abstract group, and it might be easier to think about $V$ as the space of geometric vectors that we are familiar with, and $\symup{GL}(V)$ as the group of all invertible linear transformations mapping each vector to another vector in $V$.
			...
			\begin{proof}
				...
				In essence, one needs to show that $\theta: \symup{GL}(V) \rightarrow \symup{GL}_n(\symbb{F})$ is a group homomorphism, and then that $\theta$ is bijective.
			\end{proof}
		
			It is important to remember that $\symup{GL}(V)$ is an abstract group of linear maps acting on some vector space, whereas $\symup{GL}_n(\symbb{F})$ is a concrete group of matrices.
		...
		\subsubsection{The Orthogonal and Special Orthogonal Groups}